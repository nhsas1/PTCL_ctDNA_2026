#!/bin/bash
# ============================================================
# PTCL ctDNA — BAM QC Script — Batch 3
# Author: Noor Shaban
# Date: June 2026
# Purpose: Comprehensive QC of Batch 3 BAM files (n=13)
# Run via: sbatch submit_bam_qc_batch3.sh
# ============================================================

# ── Directory paths ─────────────────────────────────────────
BAM_DIR="/scratch/alice/n/nhsas1/PTCL/BAMs_batch3"
OUT_DIR="/scratch/alice/n/nhsas1/PTCL/QC_output_batch3"
mkdir -p "$OUT_DIR"
...
echo "=================================================" >  "$REPORT"
echo "PTCL ctDNA BAM QC Report — Batch 3"              >> "$REPORT"
...
for BAM in *.bam; do
    ...
    if [ "$CHR1_LEN" = "248956422" ]; then
        ...
        REF="hg19"
        ALL_HG38=false
        echo "hg19 DETECTED    ${BAM} — WARNING" >> "$GENOME_CHECK"
...
for BAM in *.bam; do
    ...
    # ── Batch label updated for Batch 3 filenames ───────────
    if echo "$BAM" | grep -q "LB20231211"; then
        BATCH="Batch1"
    elif echo "$BAM" | grep -q "LB_2025"; then
        BATCH="Batch2"
    elif echo "$BAM" | grep -q "LB_20241212\|LB_241114"; then
        BATCH="Batch3"
    else
        BATCH="Unknown"
    fi
    ...
    # ── Sample ID extraction updated for Batch 3 ────────────
    SAMPLE_ID=$(echo "$BAM" \
        | grep -oE "LB_20241212_[0-9]+|LB_241114_[0-9]+" \
        | head -1)
...
echo "PTCL ctDNA — Samtools Coverage Report — Batch 3" > "$COVERAGE"
